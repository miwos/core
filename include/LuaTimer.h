...
namespace LuaTimer {
  ...
  uint32_t currentTick = 0; // updated by `LuaMidi`.
  ...
  int scheduleMidi(lua_State *L) {
    ...
    if (availableEventIndex == -1) {
      Logger::warn("all midi-events in use");
    ...
    // message is always the case). We can therefore use it, as long as we make
    // sure to set it back to 1 before we send the message.
    if (useTicks) {
  ...
  void install() {
    updateRef = -1;
    luaL_Reg lib[] = {{"millis", millis}, {"micros", micros}, {"ticks", ticks},
        {"_scheduleMidi", scheduleMidi}, {NULL, NULL}};
